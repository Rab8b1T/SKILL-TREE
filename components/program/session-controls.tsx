...
export function SessionControls({data, timing, save, busy, offline, draftScope, onSelect}: {
  ...
  return <Card className="mb-5 border-accent/35" data-testid="session-controls">
    ...
    {current && phase && <>
      ...
      {current === "leetcode" && phase.status !== "running" && <div className="mt-4 rounded-xl border border-line p-3"><p className="text-sm text-muted">If a prerequisite check blocks Start, return to the core lesson, repair and save your recall, then come back here.</p><Button className="mt-2" size="sm" onClick={() => onSelect("core")}>Return to core lesson</Button></div>}
