...
function ProblemList({problems,block,data,timing,save,busy,current,draftScope}:{problems:(SafeProblem|null)[];block:BlockId;data:ProgramView;timing:ProgramTiming|null;save:Save;busy:boolean;current?:BlockId;draftScope:string}){
  if(!problems.length)return <Card><p className="text-sm text-muted">{block==="core"&&data.day.core.lesson?"This core block is reserved for the lesson and recall above; there are no additional full problems.":"This phase’s content opens when you reach it with Next."}</p></Card>;
  const phase=timing?.phases[block];
  return <div className="space-y-4">{phase?.blockedReason&&<Card className="border-warning/40"><p className="text-sm text-warning">{phase.blockedReason}</p><p className="mt-2 text-sm text-muted">Your completed work is saved. Repair the required recall before starting the next problem, or choose Next to move on.</p></Card>}{problems.filter((p):p is SafeProblem=>Boolean(p)).map((p,i)=><Problem key={`${block}:${p.key}`} p={p} index={i} block={block} data={data} timing={timing} save={save} busy={busy} current={current} draftScope={draftScope}/>)}</div>;
}
...
function LessonContent({lesson,data,save,busy,draftScope}:{lesson:NonNullable<ProgramView["day"]["core"]["lesson"]>;data:ProgramView;save:Save;busy:boolean;draftScope:string}){
  ...
  const current=data.run?.timing?.currentBlock,lcPhase=data.run?.timing?.phases.leetcode;
  const canSaveRecall=current==="core"||(current==="leetcode"&&lcPhase?.status!=="running"&&!lcPhase?.problems.some(p=>p.guidedSince!==undefined));
  ...
  return <div className="mb-5 space-y-4"><Card><CardTitle><BookOpen className="mr-2 inline size-5"/>{lesson.title}</CardTitle><p className="mt-2 text-sm text-muted">{lesson.why}</p><p className="mt-2 text-xs text-faint">Teaching is part of the {data.blocks.find(b=>b.id==="core")?.minutes}-minute core block · planned {lesson.minutes}m</p><ul className="mt-3 list-inside list-disc text-sm">{lesson.outcomes.map(x=><li key={x}>{x}</li>)}</ul></Card>
    ...
    <Card><CardTitle>Close the lesson and recall</CardTitle><p className="mt-2 text-sm text-muted">{lesson.drill.prompt}</p><div className="mt-4 space-y-4"><TextField label="Teach it back in your own words" value={teachBack} onChange={setTeachBack}/>{lesson.check.map((q,i)=><div key={i}><TextField label={`${i+1}. ${q.q}`} value={answers[i]??""} onChange={v=>updateAnswer(i,v)}/>{data.run?.recallFeedback?.checks[i]&&<p className={cn("mt-2 text-xs",data.run.recallFeedback.checks[i].status==="needs-review"?"text-warning":"text-muted")}>{answers[i]!==data.run.lessonEvidence?.answers[i]?"Unsaved answer — save recall to check this change.":data.run.recallFeedback.checks[i].status==="matched"?"Saved answer: keyword check matched.":data.run.recallFeedback.checks[i].status==="needs-review"?"Saved answer: needs review. The automatic keyword check did not match; clarify your wording or ask Mentor to assess it.":"Saved answer: awaiting Mentor assessment."}</p>}{q.a&&<details className="mt-2 text-sm"><summary>Compare with the answer after your saved attempt</summary><p className="mt-2 text-muted">{q.a}</p></details>}</div>)}<TextField label="From-memory Python primitive" value={code} onChange={setCode} code/><Button disabled={busy||!canSaveRecall||teachBack.trim().length<10||code.trim().length<5||lesson.check.some((_,i)=>!answers[i]?.trim())} onClick={()=>void save({type:"lesson",teachBack,answers,primitiveCode:code}).catch(()=>{})}>Save recall evidence</Button>{data.run?.lessonEvidence&&<p className="text-sm text-muted">{data.run.recallFeedback?.message??"Evidence saved. Return online to retrieve the recall check details."}</p>}</div></Card>
  </div>;
}
