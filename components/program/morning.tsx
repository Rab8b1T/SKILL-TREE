...
import { BLOCKS,type BlockId,type ProgramAction,type ProgramView } from "@/lib/program";
...
export function ProgramMorning({initialBlock,initialDay}:{initialBlock?:BlockId;initialDay?:number}){
  ...
  const current=data.run?BLOCKS.find(b=>now>=data.run!.blocks[b.id].startedAt&&now<data.run!.blocks[b.id].endsAt)?.id:undefined;
...
function LessonContent({lesson,data,save,busy}:{lesson:NonNullable<ProgramView["day"]["core"]["lesson"]>;data:ProgramView;save:Save;busy:boolean}){
  ...
  return <div className="mb-5 space-y-4"><Card><CardTitle><BookOpen className="mr-2 inline size-5"/>{lesson.title}</CardTitle><p className="mt-2 text-sm text-muted">{lesson.why}</p><p className="mt-2 text-xs text-faint">Teaching is part of the 120-minute core block · planned {lesson.minutes}m</p><ul className="mt-3 list-inside list-disc text-sm">{lesson.outcomes.map(x=><li key={x}>{x}</li>)}</ul></Card>
