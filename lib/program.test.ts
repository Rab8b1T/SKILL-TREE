...
describe("server-owned session",()=>{
 ...
 it("keeps upsolve acceptance separate from the original round",()=>{const p=fixture();let r=createRun(p,p.days[0],NOW);r=applyAction(r,{type:"attempt",block:"review",key:"1-A",technique:"Repair"},"review-attempt",NOW+7200000);r=reconcileCf(r,[sub(3,7300)],NOW+7400000);expect(r.attempts["review:1-A"].verification).toBe("verified");expect(r.attempts["contest:1-A"]).toBeUndefined();expect(contestScore(r)).toBe(0);});
});
