...
export function applyAction(original:ProgramRun,action:ProgramAction,eventId:string,now:number):ProgramRun {
  ...
  if(action.type==="finish-contest") {
    ...
    if(now<run.blocks.review.startedAt)throw new Error("Review is locked during the contest");
    ...
    if(action.type==="attempt") {
      if(block!==action.block)throw new Error("This block is not on the clock");
      if(action.block==="core"||action.block==="leetcode")for(const topic of (p as ProgramProblem).topicIds)if(!topicEligible(run.topics,topic,"core",run.programDay,now,run.lessonEvidence,run.snapshot.diagnosticBootstrap===true))throw new Error(`Teach and check ${topic} before starting this problem`);
      if(run.attempts[key])return original;
      if(!action.technique.trim())throw new Error("Name your intended technique before coding");
      run.attempts[key]={key:action.key,block:action.block,startedAt:now,technique:action.technique,hintsUsed:0,solutionSeen:false,verification:"pending"};
    } else if(action.type==="report") {
      const a=run.attempts[key];if(!a)throw new Error("Start an attempt before recording its outcome");
      a.reported=action.reported;a.note=action.note;a.finishedAt=Math.min(now,run.blocks[action.block].endsAt);
    } else if(action.type==="hint") {
      if(now<run.blocks.review.startedAt)throw new Error("Hints stay locked until the contest ends");
      if(now<run.blocks[action.block].startedAt)throw new Error("This block has not started");
  ...
  run.processed.push(eventId);run.events.push({id:eventId,type:action.type,at:now,...("block" in action?{block:action.block,key:action.key}:{})});run.revision++;
  return run;
}
export function reconcileCf(run:ProgramRun,submissions:CfSubmission[],now:number):ProgramRun {
  ...
  for(const b of ["contest","review","core"] as const) {
    ...
    for(const p of probs) {
      ...
      const until=Math.min(out.blocks[b].endsAt,now);
      const subs=submissions.filter(s=>`${s.problem.contestId}-${s.problem.index}`===p.key&&s.creationTimeSeconds*1000>=since&&s.creationTimeSeconds*1000<until).sort((x,y)=>x.creationTimeSeconds-y.creationTimeSeconds);
      ...
      entry.submissions=subs.map(s=>({id:s.id,at:s.creationTimeSeconds*1000,verdict:s.verdict??"PENDING"}));
      const ac=subs.find(s=>s.verdict==="OK");entry.verification=ac?"verified":"pending";entry.solvedAt=ac?ac.creationTimeSeconds*1000:undefined;
      entry.wrongAttempts=subs.filter(s=>(!ac||s.creationTimeSeconds<=ac.creationTimeSeconds)&&countsAsWrongSubmission("cf",s.verdict,s.passedTestCount)).length;entry.lastCheckedAt=now;out.attempts[k]=entry;
    }
  }
...
export function publicProgram(p:Program,run:ProgramRun|null,selected:ProgramDay,now:number) {
  ...
  const reveal=Boolean(run&&now>=run.blocks.review.startedAt);
