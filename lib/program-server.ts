...
import { actionSchema, applyAction, assertContestEligible, canonicalJson, createRun, localDate, parseProgram, publicProgram, reconcileCf, sessionId, startWindow, type Program, type ProgramDay, type ProgramRun } from "./program";
...
export async function readProgramRun(userId:string,p:Program,d:ProgramDay) {
  return (await (await runCollection()).findOne({_id:runStorageId(userId,p,d.programDay)}))?.run??null;
}
export async function getProgramView(userId:string,handle:string|null|undefined,day?:number) {
  const p=await loadProgram();assertProgramHandle(p,handle);let d=programDay(p,day);
...
export function reconcileLc(run:ProgramRun,accepts:RecentLc[],now:number):ProgramRun {
  ...
  for(const p of out.snapshot.leetcode.problems){
    ...
    const ac=accepts.filter(s=>s.titleSlug===p.slug&&Number(s.timestamp)*1000>=a.startedAt&&Number(s.timestamp)*1000<Math.min(now,out.blocks.leetcode.endsAt)).sort((x,y)=>Number(x.timestamp)-Number(y.timestamp))[0];
    if(ac){a.verification="verified";a.solvedAt=Number(ac.timestamp)*1000;a.submissions=[{id:Number(ac.id),at:a.solvedAt,verdict:"OK"}];}
    a.lastCheckedAt=now;
  }
...
export async function mutateProgram(userId:string,handle:string|null|undefined,body:unknown) {
  ...
  const d=programDay(p,b.programDay as number),col=await runCollection(),id=runStorageId(userId,p,d.programDay);
  const stored=await col.findOne({_id:id}),now=Date.now();
  if(action.type==="start"){
    if(stored)return publicProgram(p,stored.run,d,now);
    ...
    return publicProgram(p,(await col.findOne({_id:id}))!.run,d,now);
  }
  if(!stored)throw new HttpError(409,"Start the day before recording work");
  if(stored.run.processed.includes(b.eventId))return publicProgram(p,stored.run,d,now);
  ...
  if(action.type==="sync"){
    if(action.source==="cf"){
      try {run=reconcileCf(run,await getUserStatusSince(p.handle,Math.floor(run.startedAt/1000)),now);}
      ...
      try {
        ...
        run=reconcileLc(run,history as unknown as RecentLc[],now);
  ...
  return publicProgram(p,run,d,now);
}
export async function programExport(userId:string,programId:string) {
  const rows=await (await runCollection()).find({ownerId:userId,programId}).sort({"run.programDay":1}).toArray();
