      ...
      <div className="mb-4 grid gap-4 sm:grid-cols-2">
        {day.practice && (
          <SessionCard
            ...
            href="/coach/legacy/practice"
            icon={AlarmClock}
            blurb={`${(day.practice.blocks ?? []).reduce((s, b) => s + b.problems.length, 0)} problems · ${(day.practice.blocks ?? []).reduce((s, b) => s + b.minutes, 0)} min · timers track engaged effort only`}
          />
        )}
        {day.contest && (
          <SessionCard
            ...
            href="/coach/legacy/contest"
            icon={Swords}
            blurb={`${day.contest.mirrors} · ${day.contest.problems.length} problems · points decay to 30%`}
          />
        )}
        {day.lesson && (
          <LinkCard
            title={day.lesson.title}
            href="/coach/legacy/lesson"
            icon={GraduationCap}
            blurb={`${day.lesson.topic} · ${day.lesson.minutes} min · ${day.lesson.steps.length} steps`}
          />
        )}
        {day.leetcode && (
          <LinkCard
            title={day.leetcode.title}
            href="/coach/legacy/leetcode"
            icon={Code2}
            blurb={`${day.leetcode.problems.length} problems · ${day.leetcode.minutes} min · interview track`}
          />
        )}
      </div>
